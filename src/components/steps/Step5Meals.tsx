      ...
      <div className="flex flex-wrap gap-2">
        {LUNCH_BASES.map(({ v, e }) => (
          <Pill
            ...
            label={v}
            emoji={e}
            selected={lunch.base === v}
            onClick={() => setLunchBase(v)}
          />
        ))}
      </div>
      {lunch.base === "Roti" && (
          ...
          <div className="flex flex-wrap gap-2">
            {ROTI_TYPES.map((r) => (
              <Pill
                key={r}
                label={r}
                selected={lunch.rotiType === r}
                onClick={() => setLunchRotiType(r)}
              />
            ))}
          </div>
        </div>
      ...
      <div className="flex flex-wrap gap-2">
        {DINNER_BASES.map(({ v, e }) => (
          <Pill
            ...
            label={v}
            emoji={e}
            selected={dinner.base === v}
            onClick={() => setDinnerBase(v)}
          />
        ))}
      </div>
    </MealBlock>
      ...
      <div className="flex flex-wrap gap-2 mb-[14px]">
        {SNACK_TYPES.map(({ v, e }) => (
          <Pill
            ...
            label={v}
            emoji={e}
            selected={snacks.types.includes(v)}
            onClick={() => toggleSnackType(v)}
          />
        ))}
      </div>
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {SNACK_FREQ.map((f) => (
            <Pill
              key={f}
              label={f}
              selected={snacks.freq === f}
              onClick={() => setSnackFreq(f)}
            />
          ))}
        </div>
      </div>
